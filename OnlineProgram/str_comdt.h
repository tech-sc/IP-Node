...
//SS9100(PH1再開)対応 2006.06
    ...
    INT			operation_mode;			/* 動作モード(0：保守モード/１：通常モード) */
    char		ipl_ver[8];				/* IPLバージョン情報 */
    char		onl_ver[8];				/* ONLバージョン情報 */
    char		hard_ver_w[2];			/* ハードバージョン(オンライン通知用) */
    char		fpga_ver[2];				/* FPGAバージョン(オンライン通知用) */
    BYTE		wdt_mode;					/* wdtモード(0：起動/1：未起動) */
    BYTE		dummy[3];					/* ダミーエリア */
...
typedef struct {  // IP-CS(SS9100_PH1再開) 追加
    ...
    BYTE        onl_restart;            /*#  オンライン後リスタートフラグ#*//* IPCS_V4 <PD><ADD> */
    BYTE        mnt_reset;              /*#  保守モード起動要求フラグ#*//* IPCS_V4 <PD><ADD> */
    BYTE        reserve[10];            /*# 予備 #*//* IPCS_V4 <PD><CHG> */
}FLS_CA_DATA;
